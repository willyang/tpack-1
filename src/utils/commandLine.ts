...
/**
 * 将日志内容按最大列数拆成多行
 * @param content 要处理的日志内容
 * @param columns 布局的最大列数
 */
export function splitLog(content: string, columns = process.stdout.columns || Infinity) {
	const result: string[] = []
	let left = 0
	let currentWidth = 0
	let leftBound = 0
...
/**
 * 如果日志内容显示宽度超过最大值，则将中间部分替换为省略号
 * @param content 要处理的日志内容
 * @param columns 布局的最大列数
 * @param ellipsis 使用的省略号
 */
export function ellipsisLog(content: string, columns = process.stdout.columns || Infinity, ellipsis = "...") {
	// 删除省略号本身的宽度
	...
	for (let i = 0; i < rows.length; i++) {
		...
		for (let j = 0; j < row.length; j++) {
			...
			if (actualWidth > columnWidth) {
				cell = ellipsisLog(cell, columnWidth, ellipsis)
...
export function showCursor(stream = process.stdout) {
	if (stream.isTTY) {
		stream.write("\u001b[?25h")
		if ((stream as any).__showCursor__) {
			(stream as any).__showCursor__()
			delete (stream as any).__showCursor__
		}
	}
}
...
export function hideCursor(stream = process.stdout) {
	if (stream.isTTY) {
		...
		if (!(stream as any).__showCursor__) {
			(stream as any).__showCursor__ = require("signal-exit")(() => {
				delete (stream as any).__showCursor__
				showCursor(stream)
			})
		}
