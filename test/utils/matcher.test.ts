...
export namespace matcherTest {
	...
	export function matchTest() {
		...
		assert.strictEqual(matcher.match("foo.js", /foo\.js/), true)
		assert.strictEqual(matcher.match("goo.js", createMatcher(createMatcher(["!goo.js", /foo\.js/]))), false)
		assert.strictEqual(matcher.match("foo.js", createMatcher(createMatcher(["!goo.js", /foo\.js/]))), true)
		const mg = createMatcher(/foo\.js/)
		mg.exclude("goo.js")
		...
		assert.strictEqual(matcher.match(resolve("foo"), "./", { cwd: process.cwd() }), true)
		if (sep === "\\") {
			assert.strictEqual(matcher.match("E:\\foo", "E:\\foo", { cwd: process.cwd() }), true)
			assert.strictEqual(matcher.match("E:\\foo\\goo.txt", "E:\\foo", { cwd: process.cwd() }), true)
			assert.strictEqual(matcher.match("E:\\foo\\goo.txt", "E:\\foo\\*", { cwd: process.cwd() }), true)

			assert.strictEqual(matcher.match("C:\\", "C:\\", { cwd: process.cwd() }), true)
			assert.strictEqual(matcher.match("C:\\", "../../", { cwd: process.cwd() }), false)
		} else {
			assert.strictEqual(matcher.match("/user/local", "/user/local", { cwd: process.cwd() }), true)
			assert.strictEqual(matcher.match("/user/local/file.txt", "/user/local", { cwd: process.cwd() }), true)
			assert.strictEqual(matcher.match("/user/local/file.txt", "/user/local/*", { cwd: process.cwd() }), true)
		}

		assert.strictEqual(matcher.match(resolve("bdir/x/x"), /^bdir\/x\/x$/, { cwd: process.cwd() }), true)
		assert.strictEqual(matcher.match(resolve("bdir/x/x"), "bdir/[x]/\\x", { cwd: process.cwd() }), true)
		assert.strictEqual(matcher.match(resolve("bdir/x/x"), path => path === resolve("bdir/x/x"), { cwd: process.cwd() }), true)
	...
	export namespace micromatchTest {
		...
		export function dotglobTest() {
			...
			assert.strictEqual(matcher.match(".bar.baz/", ".*.*"), false)
		...
		export function globstarTest() {
			...
			assert.strictEqual(matcher.match("foo/", "foo/**/"), true)
			assert.strictEqual(matcher.match("foo/bar", "foo/**/"), false)
			assert.strictEqual(matcher.match("foo/bazbar", "foo/**/"), false)
			assert.strictEqual(matcher.match("foo/barbar", "foo/**/"), false)
			assert.strictEqual(matcher.match("foo/bar/baz/qux", "foo/**/"), false)
			assert.strictEqual(matcher.match("foo/bar/baz/qux/", "foo/**/"), true)
		}

		export function negationTest() {
			// https://github.com/micromatch/micromatch/blob/master/test/fixtures/negation.txt
		...
		export function patternsTest() {
			// https://github.com/micromatch/micromatch/blob/master/test/fixtures/patterns.txt
			...
			assert.strictEqual(matcher.match("a.,ts", "a.,ts"), true)
			assert.strictEqual(matcher.match("x\\", "[x\\"), false)
			assert.strictEqual(matcher.match(resolve("foo"), resolve("foo"), { cwd: process.cwd() }), true)
			assert.strictEqual(matcher.match(resolve("[]"), resolve("[]"), { cwd: process.cwd() }), true)
	...
	export function baseTest() {
		assert.strictEqual(createMatcher("path/to/*.js").base, "path/to")

		assert.strictEqual(createMatcher().base, null)
		assert.strictEqual(createMatcher("foo").base, "")
		assert.strictEqual(createMatcher("root/path/to/*.js").base, "root/path/to")
		assert.strictEqual(createMatcher("/foo/*.js").base, "/foo")
		assert.strictEqual(createMatcher("*.js").base, "")
		assert.strictEqual(createMatcher("**/*.js").base, "")
		assert.strictEqual(createMatcher("path/?").base, "path")
		assert.strictEqual(createMatcher("path/foo[ab]").base, "path")
		assert.strictEqual(createMatcher("path/*").base, "path")
		assert.strictEqual(createMatcher("path/foo*").base, "path")
		assert.strictEqual(createMatcher("path/**/*").base, "path")
		assert.strictEqual(createMatcher("path/**/subdir/foo.*").base, "path")
		assert.strictEqual(createMatcher("foo/").base, "foo")
		assert.strictEqual(createMatcher(["foo/goo", "foo/foo"]).base, "foo")
		assert.strictEqual(createMatcher(["foo/**/*.js", "foo/**/*.css"]).base, "foo")
		assert.strictEqual(createMatcher(/foo/).base, "")
		assert.strictEqual(createMatcher(["foo/", "!foo/"]).base, "foo")
		assert.strictEqual(createMatcher("./").base, "")
		assert.strictEqual(createMatcher(".").base, "")
		assert.strictEqual(createMatcher("").base, "")
		assert.strictEqual(createMatcher("fo\\o/").base, "foo")

		assert.strictEqual(createMatcher(["../foo/**/*.js", "foo/**/*.css"], { cwd: resolve("root") }).base, resolve("."))
		assert.strictEqual(createMatcher(["../foo/**/*.js", /foo/], { cwd: resolve("root") }).base, resolve("."))
		assert.strictEqual(createMatcher(["fo\\o", () => true], { cwd: "src" }).base, "src")
		assert.strictEqual(createMatcher([1 as any], { cwd: "src" }).base, null)
		assert.strictEqual(createMatcher("foo/", { cwd: resolve("src") }).base, resolve("src/foo"))
		assert.strictEqual(createMatcher("/foo/", { cwd: resolve("src") }).base, "/foo")
		assert.strictEqual(createMatcher(resolve("foo") + sep, { cwd: resolve("src") }).base, resolve("foo"))

		assert.strictEqual(createMatcher(createMatcher(createMatcher(createMatcher("fo\\o/")))).base, "foo")
		assert.strictEqual(createMatcher(createMatcher(createMatcher(createMatcher(() => false)))).base, "")
		assert.strictEqual(createMatcher(createMatcher(createMatcher(createMatcher([() => false, () => false, () => false])))).base, "")

		if (sep === "\\") {
			assert.strictEqual(createMatcher("C:\\", { cwd: process.cwd() }).base, "C:")
			assert.strictEqual(createMatcher(["C:\\", "D:\\"], { cwd: process.cwd() }).base, null)
		}
	}

	export function getBasesTest() {
		assert.deepStrictEqual(createMatcher(["foo/", "goo/"]).getBases(), ["foo", "goo"])

		assert.deepStrictEqual(createMatcher().getBases(), [])
		assert.deepStrictEqual(createMatcher("foo/").getBases(), ["foo"])
		assert.deepStrictEqual(createMatcher(["foo/", "foo/goo"]).getBases(), ["foo"])
		assert.deepStrictEqual(createMatcher(["foo/goo/hoo", "foo/goo/"]).getBases(), ["foo/goo"])
		assert.deepStrictEqual(createMatcher(["foo/goo/hoo", "foo/goo"]).getBases(), ["foo"])
		assert.deepStrictEqual(createMatcher(["foo/goo/hoo", /foo/]).getBases(), [""])

		assert.deepStrictEqual(createMatcher(["../foo/goo", "../foo/"], { cwd: process.cwd() }).getBases(), [resolve("../foo")])
		assert.deepStrictEqual(createMatcher(["foo/goo/hoo", "../"], { cwd: process.cwd() }).getBases(), [resolve("..")])
		assert.deepStrictEqual(createMatcher(["../foo/goo/hoo", "../"], { cwd: process.cwd() }).getBases(), [resolve("..")])
	}

	export function baseOfTest() {
		assert.strictEqual(createMatcher("foo/").baseOf("foo/goo.js"), "foo")

		assert.strictEqual(createMatcher(["foo/", "goo/"]).baseOf("foo/goo.js"), "foo")
		assert.strictEqual(createMatcher(["foo/", "foo/goo"]).baseOf("foo/goo.js"), "foo")
		assert.strictEqual(createMatcher(["foo/goo/hoo", "foo/goo/"]).baseOf("foo/goo/hoo.js"), "foo/goo")
		assert.strictEqual(createMatcher(["foo/goo/hoo", "foo/goo"]).baseOf("foo/goo/hoo.js"), "foo/goo")
	}

	export function relativeTest() {
		assert.strictEqual(createMatcher("src").relative("src/x.js"), "src/x.js")

		assert.strictEqual(createMatcher("src/pages/").relative("src/pages/x.js"), "x.js")
		assert.strictEqual(createMatcher("src/pages").relative("src/pages/x.js"), "pages/x.js")
		assert.strictEqual(createMatcher(["src/pages/", "src"]).relative("src/pages/x.js"), "x.js")
		assert.strictEqual(createMatcher(["src/pages", "src"]).relative("src/pages/x.js"), "pages/x.js")
		assert.strictEqual(createMatcher(["src", "src/pages/"]).relative("src/pages/x.js"), "x.js")
		assert.strictEqual(createMatcher(["src", "src/pages/"]).relative("src/x.js"), "src/x.js")
		assert.strictEqual(createMatcher(["src", "src/pages"]).relative("src/pages/x.js"), "pages/x.js")
		assert.strictEqual(createMatcher(["src/", "src/pages/"]).relative("src/x.js"), "x.js")
		assert.strictEqual(createMatcher(["src/pages/", "src/"]).relative("src/x.js"), "x.js")
		assert.strictEqual(createMatcher(["src/pages/", "src/"]).relative("src"), "")

		assert.strictEqual(createMatcher(["src/s*.js"]).relative("src/s1.js"), "s1.js")
	}
	...
	function createMatcher(include?: matcher.Pattern, options?: matcher.MatcherOptions) {
		const m = new matcher.Matcher(options)
		m.include(include)
		return m
	}
