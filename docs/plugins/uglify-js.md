# JS 压缩插件
使用 [UglifyJS](https://github.com/mishoo/UglifyJS) 混淆、压缩或格式化 JS。

## 安装
此插件已内置，无需安装。
启用优化时，如果项目里存在 `.js` 文件，TPack 会自动安装 uglify-js 并压缩 JS。

或者你也可以手动安装 uglify-js：`npm install uglify-js --save-dev`。
...
```
完整最新的配置请参考: [官方文档](https://github.com/mishoo/UglifyJS2)。

## 使用
使用 `tpack -p` 或在配置中添加 `{optimize: true}` 启用压缩。

所有 JS 文件，包括由 JSX、TS 等编译生成的 JS，HTML 内联的 JS 都会被压缩。

如果希望排除 HTML 内联的 JS，可以在匹配的规则添加 `!*|*.js`。